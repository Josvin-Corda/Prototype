...
public class PlayerCarProgression : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        // Modern dark translucent GUI box on the right side of the screen
        GUI.backgroundColor = new Color(0.12f, 0.12f, 0.18f, 0.9f);
        GUILayout.BeginArea(new Rect(Screen.width - 320f, 10f, 300f, 250f), GUI.skin.box);
        ...
        GUILayout.Label("VR Player Car Controls", titleStyle);
        GUILayout.Space(5f);

        // Status Readout
        GUILayout.Label($"Car State: <color=yellow>{playerCarState}</color>", GetRichTextLabelStyle());
        GUILayout.Label($"Player Position: {(isPlayerInsideCar ? "<color=green>Inside Car</color>" : "<color=cyan>Outside Car</color>")}", GetRichTextLabelStyle());
        GUILayout.Label($"NPC Spawner: {(spawner != null && spawner.isSpawningPaused ? "<color=orange>Paused</color>" : "<color=green>Active</color>")}", GetRichTextLabelStyle());
        GUILayout.Space(10f);

        // Instructions
        GUILayout.Label("<b>Keyboard Controls:</b>", GetRichTextLabelStyle());
        GUILayout.Label("• <b>[P]</b> Spawn & Register player 'TestCar'", GetRichTextLabelStyle());
        GUILayout.Label("• <b>[G]</b> Enter/Exit (Board) vehicle", GetRichTextLabelStyle());
        
        // Progress prompt based on state
        string progressAction = "Unavailable (Moving/Exited)";
        bool canProgress = false;
        if (playerCarState == ValetState.AtDropOff)
        {
            progressAction = "<color=green>Send Car to Park Spot</color>";
            canProgress = true;
        }
        else if (playerCarState == ValetState.Parked)
        {
            progressAction = "<color=green>Recall Car to Pick-Up Spot</color>";
            canProgress = true;
        }
        else if (playerCarState == ValetState.AtPickUp)
        {
            progressAction = "<color=green>Send Car to Exit Spot</color>";
            canProgress = true;
        }

        GUILayout.Label($"• <b>[Enter]</b> Proceed: {progressAction}", GetRichTextLabelStyle());
        
        if (canProgress)
        {
            GUILayout.Space(5f);
            GUI.backgroundColor = Color.green;
            if (GUILayout.Button("Proceed to Next Step", GUILayout.Height(30f)))
            {
                ProgressValetState();
            }
        }
    ...
    private GUIStyle GetRichTextLabelStyle()
    {
